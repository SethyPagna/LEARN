...
**Revision audited:** `5f06f9e1` (`main`) · **Date:** 2026-09-21
...
| Status | Count | Items |
| --- | --- | --- |
| ✅ **PASS** | 12 | 1, 2, 3, 6, 7, 8, 9, 11, 13, 15, 16, 17 |
| ⚠️ **PARTIAL** | 6 | 4, 5, 10, 14, 18, 19 |
| ❌ **FAIL** | 2 | 12, 20 |
...
**Three findings need action, in this order:**
1. 🔴 **Item 5 — hardcoded fallback encryption key.** Provider secrets are encrypted with a publicly-known literal if `LEARN_SECRET_KEY`/`AUTH_SECRET` is unset in production. Highest severity in the report.
2. 🟠 **Item 6 — CSRF check bypassed on 5 mutation routes.** They authenticate via `getCurrentUser()` instead of `requireApiUser()`, skipping the `hasTrustedOrigin()` guard.
3. 🟠 **Items 12 + 20 — no bot protection and no dependency scanning.** Combined with 100k PBKDF2 iterations (Item 10), credential stuffing is cheaper than it should be.
...
### 5. Encrypt Sensitive Data at Rest — ⚠️ PARTIAL 🔴
...
1. Fail closed in production: if `NODE_ENV === "production"` and no master key is set, **throw** rather than derive from the literal.
2. Rotate any provider secrets encrypted while the fallback may have been active.
3. Add a startup assertion that the master key is present and non-default.

---

## Category B — Auth, Access & Sessions

### 6. Enforce Server-Side Auth — ⚠️ PASS *with a CSRF gap*
...
**Action:** migrate the four mutation routes to `requireApiUser(request)`. Trivial change, closes the gap and removes the inconsistency.
...
### 18. Add Security Headers — ⚠️ PARTIAL
...
**Gaps:**
1. 🔴 `script-src 'self' 'unsafe-inline' 'unsafe-eval'` — `'unsafe-inline'` and `'unsafe-eval'` together largely defeat CSP as an XSS mitigation. Next.js needs `'unsafe-inline'` for its bootstrap script, but `'unsafe-eval'` is typically only required in development.
2. 🟠 **No `Strict-Transport-Security`** header (see Item 19).
3. `Permissions-Policy` denies `microphone=()` — **but the app has WebRTC calling and voice transcription**, both of which need the microphone. This header will block those features in the browser. (Either the header is wrong, or the features are broken in production — worth confirming.)

**Action:** drop `'unsafe-eval'` from production CSP (keep it dev-only); move to a nonce-based `script-src` if feasible; add HSTS; fix the `microphone` directive to match the actual product.

### 19. Force HTTPS — ⚠️ PARTIAL

| Evidence | Detail |
| --- | --- |
| `next.config.mjs:20` | `upgrade-insecure-requests` in CSP ✅ |
| `login/route.ts:39` | Cookie `secure` flag set when the request is HTTPS ✅ |
| HSTS | **Absent** — no `Strict-Transport-Security` header anywhere |

Transport security currently depends on Cloudflare's edge defaults rather than an explicit application policy. Without HSTS, the first request in a session can still be downgraded (SSL-strip) if the user types the bare hostname.

**Action:** add `Strict-Transport-Security: max-age=63072000; includeSubDomains; preload` — ideally at the Cloudflare edge, or in `next.config.mjs` `headers()`.
...
1. **Item 5** — fail closed when no master key is set in production; rotate any secrets encrypted under the fallback. *(Credential exposure.)*
2. **Item 18.3** — fix `Permissions-Policy: microphone=()` which contradicts the app's own calling/voice features. *(Broken functionality + misleading security config.)*
3. **Item 6** — move the 4 mutation routes (`import`, `files` POST/DELETE, `automation/run`) to `requireApiUser()` so CSRF protection applies. *(One-line change per route.)*

### 🟠 Short term (this month)
4. **Item 20** — add `pnpm audit` to CI + Dependabot. Remove the 17 unused dependencies.
5. **Item 18.1 + 19** — remove `'unsafe-eval'` from production CSP; add HSTS.
