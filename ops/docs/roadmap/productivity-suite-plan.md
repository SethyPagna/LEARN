...
- 2026-06-02: Added a live Cloudflare smoke check for route status, deployed Tailwind CSS utilities, and favicon delivery so the Worker can be verified after every deploy.
...
- 2026-06-02: Added live security-header checks to the Cloudflare smoke gate for frame denial, MIME sniffing protection, and core CSP directives already served by the Worker.
- 2026-06-02: Added the source-layout migration rule: when files or folders are reorganized, update TypeScript aliases, test discovery, docs, Windows wrappers, Cloudflare configs, and deployment scripts in the same pass, then verify before pushing live.
