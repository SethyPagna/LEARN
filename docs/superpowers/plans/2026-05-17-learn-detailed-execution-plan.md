...
**Progress note (2026-05-17):** Phase 4 has started with tested shared tone/control helpers in `lib/design-system.ts`, reusable `StatusPill` and `ControlButton` primitives in `components/learn/ui.tsx`, Dashboard status/metric tone logic wired to the shared helpers, and AI Tutor status/menu/gateway/result controls moved off one-off color helpers.
