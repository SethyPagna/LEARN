...
export function controlButtonClasses(input: {
  ...
  const base = `${size} inline-flex items-center justify-center gap-2 rounded-lg border font-semibold transition focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-55`
...
export function menuSurfaceClasses() {
  return "rounded-xl border border-border bg-popover p-2 text-popover-foreground shadow-lift"
}
