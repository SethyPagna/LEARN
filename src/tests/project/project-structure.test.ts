...
test("tracked docs stay grouped by topic", () => {
  const trackedDocsEntries = listTrackedFiles()
    .filter((filePath) => filePath.startsWith("ops/docs/"))
...
test("Tailwind global styles explicitly scan project source files", () => {
  for (const stylesheetPath of ["src/app/globals.css", "src/app/styles/globals.css"]) {
...
test("Cloudflare smoke script verifies live routes and CSS", () => {
  ...
  assert.match(smokeScript, /REQUIRED_RESPONSE_HEADERS/)
  assert.match(smokeScript, /x-frame-options/)
  assert.match(smokeScript, /x-content-type-options/)
  assert.match(smokeScript, /frame-ancestors 'none'/)
  assert.match(smokeScript, /upgrade-insecure-requests/)
})
