...
test("social moment options guide posting without long copy", () => {
  const empty = buildSocialMomentOptions({ connectionCount: 0, threadCount: 0 })
  const active = buildSocialMomentOptions({ connectionCount: 3, threadCount: 2 })

  assert.deepEqual(empty.map((option) => option.label), ["Win", "Question", "Resource", "Milestone"])
  assert.equal(empty.find((option) => option.id === "question")?.recommended, true)
...
test("chat inbox shortcuts route to the most useful thread views", () => {
  const summary = summarizeChatWorkspace([
    { title: "#study-help - React", last_message: "[question] Can @alex check /studio notes?" },
    { title: "#wins - Streak", last_message: "[win] Finished reviews" },
    { title: "#general - Links", last_message: "[saved] bookmark this resource" },
  ])
  const shortcuts = buildChatInboxShortcuts(summary)

  assert.deepEqual(shortcuts.map((shortcut) => shortcut.label), ["All", "Help", "Wins", "Saved", "Mentions", "Studio"])
  assert.deepEqual(shortcuts.find((shortcut) => shortcut.id === "help"), { id: "help", label: "Help", count: 1, filter: "questions", query: "", recommended: true })
  assert.equal(shortcuts.find((shortcut) => shortcut.id === "mentions")?.query, "@")
  assert.equal(shortcuts.find((shortcut) => shortcut.id === "studio")?.query, "/studio")
})
...
test("normalizeSocialInviteDraft validates email and role for secure invites", () => {
  assert.deepEqual(socialInviteRoleOptions.map((option) => option.value), ["learner", "admin"])
  assert.equal(normalizeSocialInviteRole("admin"), "admin")
  assert.equal(normalizeSocialInviteRole("owner"), "learner")
...
test("social unified search command keeps one obvious next action", () => {
  ...
  const chatOnly = buildSocialUnifiedSearchCommand({ connectionCount: 2, groupResultCount: 0, peopleResultCount: 0, query: "review", roomCount: 0, threadResultCount: 2 })
  ...
  assert.equal(chat.action, "people")
  assert.equal(chatOnly.action, "chat")
...
test("social home lanes map familiar social actions to existing routes", () => {
  const empty = buildSocialHomeLanes({ battleCount: 0, connectionCount: 0, roomCount: 0, spaceCount: 0, threadCount: 0 })
  const ready = buildSocialHomeLanes({ battleCount: 1, connectionCount: 3, roomCount: 2, spaceCount: 1, threadCount: 4 })

  assert.deepEqual(empty.map((lane) => lane.label), ["Friends", "Chats", "Moments", "Groups", "Calls"])
  assert.deepEqual(empty[0].target, { kind: "command", value: "people" })
...
test("social call modes expose voice video group focus and battle options", () => {
  const empty = buildSocialCallModes({ battleCount: 0, connectionCount: 0, roomCount: 0 })
  const ready = buildSocialCallModes({ battleCount: 0, connectionCount: 3, roomCount: 1 })

  assert.deepEqual(empty.map((mode) => mode.label), ["Voice call", "Video call", "Group call", "Focus room", "Battle huddle"])
  assert.equal(empty.find((mode) => mode.id === "focus")?.recommended, true)
  assert.equal(ready.find((mode) => mode.id === "voice")?.target, "rooms")
  assert.equal(ready.find((mode) => mode.id === "battle")?.recommended, true)
  assert.equal(ready.find((mode) => mode.id === "group")?.badge, "3 friends")
})
